...
- **Environment**, if the gradio app runs in a slightly different environment than when running via the `./planexe` CLI, then the child process may behave differently. I have verified that the parent process and child process runs with the same environment variables.
...
```bash
RUN_ID_DIR=/absolute/path/to/PlanExe_20250209_030626 ./planexe create_plan --plan-file /absolute/path/to/PlanExe_20250209_030626/001-2-plan.txt --output-dir /absolute/path/to/PlanExe_20250209_030626
```

Or if you know the original plan text:

```bash
./planexe create_plan \
    --plan-text "your original plan text here" \
    --output-dir ./planexe-outputs/2026-03-16/MyStuckPlan
```
